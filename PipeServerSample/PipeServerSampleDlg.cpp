...
CPipeServerSampleDlg::CPipeServerSampleDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_PIPESERVERSAMPLE_DIALOG, pParent)
	, m_Log(_T(""))
{
...
BEGIN_MESSAGE_MAP(CPipeServerSampleDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_CONNECT, &CPipeServerSampleDlg::OnButtonConnect)
END_MESSAGE_MAP()


// CPipeServerSampleDlg 消息处理程序
...
void CPipeServerSampleDlg::OnButtonConnect()
{
	...
	{
		m_Log = "调用 CreatePipe 函数创建匿名管道失败！";
		UpdateData(FALSE);
		return;
	}

	STARTUPINFO si;		// 定义启动结构
	PROCESS_INFORMATION pi;		// 定义进程信息结构
